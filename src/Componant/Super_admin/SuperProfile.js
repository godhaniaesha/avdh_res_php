...
function SuperProfile() {
    ...
    useEffect(() => {
        ...
        //     axios.post(`http://localhost:8000/api/getUser/${id}`)
        //         .then(response => {
    ...
    const getUsers = async () => {
        ...
        // Fetch user data from the backend using the ID\
        var response = await axios.post(`http://localhost/avadh_api/super_admin/profile/change_profile.php`,{}, {
            headers: {
    ...
    const handleProfileSave = async (e) => {
        ...
        if (imageInput.files.length > 0) {
            updatedData.image = imageInput.files[0]; // Get the file name from input
        }
        ...
        try{
            ...
            var response =await axios.post(`http://localhost/avadh_api/super_admin/profile/change_profile.php`, formData1, {
                headers: {
                    Authorization: `Bearer ${token}`,
